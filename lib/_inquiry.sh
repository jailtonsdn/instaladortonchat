...
get_link_git() {
  ...
  print_banner
  printf "${WHITE} 💻 Insira o link do GITHUB do seu Tonchat que deseja instalar:${GRAY_LIGHT}"
  printf "\n\n"
  read -p "> " link_git
}
...
inquiry_options() {
  ...
  print_banner
  printf "${WHITE} 💻 Bem vindo(a) ao Gerenciador Tonchat - Atendimento sem limites, selecione abaixo a proxima ação!${GRAY_LIGHT}"
  printf "\n\n"
  printf "   [0] Instalar Tonchat\n"
  printf "   [1] Atualizar Tonchat\n"
  printf "   [2] Deletar Tonchat\n"
  printf "   [3] Bloquear Tonchat\n"
  printf "   [4] Desbloquear Tonchat\n"
  printf "   [5] Alter. dominio Tonchat\n"
  printf "\n"
  read -p "> " option
